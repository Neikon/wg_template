...
**Plan de acción:**
- [x] **1.1** Crear `src/lib/game/votacion/` con `types.ts`, `engine.ts`, `Votacion.svelte`
  siguiendo `GameModule` (reducer puro, versionado +1 por transición).
- [x] **1.2** Registrarlo en `src/lib/game/registry.ts` sin tocar `currentGame`.
- [x] **1.3** Añadir `tests/unit/votacion.test.ts` (crear → votar → doble voto ignorado),
  espejo de `tests/unit/engine.test.ts`.
- [x] **1.4** Escribir `docs/NUEVO-JUEGO.md`: contrato `GameModule`, reglas del reducer
...
- [x] **1.5** Verificación: `npm run check`, `npm run test`, `npm run build`,
...
**Plan de acción:**
- [x] **2.1** Añadir al protocolo `Msg` (`src/lib/net/types.ts`) el campo `juegoId` en
  `stateSync` y en la acción `startGame` (p. ej. `{ t:'startGame', juego }`).
- [x] **2.2** Refactor `Room.svelte`: sustituir imports de trivia por
...
- [x] **2.3** Lobby: selector de juego (lista de `Object.keys(registry)`) visible para
...
- [x] **2.4** Invitados: aceptan el `juegoId` del host vía `stateSync`; si no lo tienen
  registrado, mostrar "juego no disponible" en vez de romper.
- [x] **2.5** Tests: unitario de resolución del registry + e2e (crear sala con
  `?juego=votacion`, visible su vista; volver a trivia).
- [x] **2.6** Verificación completa en local + prueba manual en navegador (2 pestañas).
...
**Plan de acción:**
- [x] **3.1** Extender `TriviaState` con `config: { numPreguntas, segundos, categoria }`
  y la acción `startGame` con esos campos (con valores por defecto = comportamiento actual).
- [x] **3.2** Lobby (host): 3 controles (número, segundos, selector de categoría según
  las disponibles en `questions.ts`, reorganizadas por tema).
- [x] **3.3** Engine: `startGame` aplica la config; `tick`/`nextQuestion` la respetan
  (fin cuando `preguntaIdx >= numPreguntas`).
- [x] **3.4** Tests unitarios: config por defecto intacta + partida corta (p. ej. 2
  preguntas, 5 s) de principio a fin vía reducer.
- [x] **3.5** e2e: configurar partida corta como host y verificar que termina en 2 preguntas.
- [x] **3.6** Verificación completa en local antes de subir.
...
**Plan de acción (hecho 2026-09-04, commit punto 4):**
- [x] **4.1** En `Room.svelte`, extraer la inicialización a `setup(salaId)` con su
...
- [x] **4.2** Alternativa/complemento en `App.svelte`: forzar remontaje con
  `{#key salaId}<Room />{/key}`. Elegir la más simple que pase los tests.
- [x] **4.3** Revisar `initRoom`/`roomStore`: no reimponer `salaId` viejo del store
  sobre el recién parseado.
- [x] **4.4** e2e de regresión: crear sala A → salir → crear sala B → el lobby muestra
  el código B, 1/20 jugadores y enlace de B.
- [x] **4.5** Verificación completa en local antes de subir.

**Solución aplicada:** `{#key salaId}` en `App.svelte` (remontaje por sala) + en
`Room.svelte` el `salaId` parseado ya no se sincroniza desde el store (la suscripción
disparaba con datos de la sala anterior y lo pisaba antes del `initRoom`) +
`gameStore` reseteado al entrar.
...
**Plan de acción:**
- [x] **5.1** Nuevo spec `tests/e2e/dos-jugadores.spec.ts`: contexto A crea sala
  (extrae `salaId` de la URL), contexto B abre `#/sala/<id>` como invitado.
- [x] **5.2** Aserciones: B ve su nombre en la lista de A; A empieza; ambos ven
  "Pregunta 1"; B responde; A ve "1/2 han respondido".
- [x] **5.3** Hacerlo tolerante a red: reintentos/timeouts amplios y `test.skip` si no
  hay conectividad a trackers (variable `E2E_P2P=1` para exigirlo en local con red).
- [x] **5.4** Documentar en el plan/spec que la red P2P real no se puede verificar en
  contenedores sin salida externa (como este), solo en local/CI con red.
- [x] **5.5** Verificación: `test:e2e` en verde en entorno con red antes de subir.
